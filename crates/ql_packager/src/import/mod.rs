use ql_core::{
    GenericProgress, Instance, IntoIoError, ListEntry, Progress,
...
pub async fn import_instance(
    ...
    if let Ok(mmc_pack) = fs::read_to_string(&try_mmc).await {
        Ok(Some((
            multimc::import(download_assets, temp_dir, &mmc_pack, sender.map(Arc::new)).await?,
            CurseforgeNotAllowed::new(),
        )))
    } else if exists(&try_qmp).await {
        qmp::import(&zip_path, download_assets, sender.map(Arc::new)).await
    } else {
